...
class EncoderLSTM(nn.Module):
    def __init__(self, inputSize, hiddenSize, nLayers = 3, dropProb = 0):
    ...
    def init_hidden(self, batchSize = 1):
        return (torch.zeros(self.nLayers, batchSize, self.hiddenSize, device=torch.device("cuda")),
                torch.zeros(self.nLayers, batchSize, self.hiddenSize, device=torch.device("cuda")))


class LuongDecoder(nn.Module):
    def __init__(self, hiddenSize, outputSize, attention, nLayers=3, dropProb=0.1):
